...
**Bug 5 — Нові релізи не отримували ID, тому треки не зберігались**

При створенні нового релізу код викликав `upsert.mutateAsync(payload)` через `useUpsertMutation`, який використовував `upsert(item).select().maybeSingle()`. Для нового запису без `id` цей виклик міг повернути `null` або об'єкт без `id`. Тоді `releaseId = saved?.id = undefined`, умова `if (releaseId || editingId)` = `false`, і `saveTracks` ніколи не викликався — реліз створювався без треків, аудіофайли губились.

Виправлення: для нових релізів тепер використовується прямий `supabase.from('releases').insert(payload).select('id').single()`, який гарантовано повертає ID нового запису. Для існуючих релізів залишається `upsert` з `editingId`. Додано імпорт `supabase` у файл.
